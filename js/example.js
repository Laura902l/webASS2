...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (isLoggedIn === "true") {
        var storedUserDetails = JSON.parse(localStorage.getItem("userDetails")) || {};
        var userEmail = storedUserDetails.userEmail;
        ...
        if (userEmail) {
            ...
            if (purchaseData && purchaseData.userEmail !== userEmail) {
                ...
                purchaseDataDiv.innerHTML = `
                    <h2 style="text-align: center">Last purchase history</h2>
                    <hr>
                    <p style="font-size: 20px; text-align: center">Total: <strong>${total} KZT</strong></p>
                    <ul style="font-size: 20px;">
                        <p>Date: <strong>${formattedTimestamp}</strong></p>
                        ${Object.keys(cart).map(productName => `<li><p>Product name: ${productName} x${cart[productName].quantity}</p> Price: ${cart[productName].total.toFixed(0)} KZT</li><br><hr>`).join('')}
                    </ul>`;
            }
        }
    } else {
        // User is not logged in, hide the purchaseData element
